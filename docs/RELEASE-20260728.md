...
### Single-agent (np=1) — Original Qwen control vs Treebeard

Same stock Q5, B70, sequential tasks (`results/private-verification-20260728/single-agent-ab-20260728T213156Z/`):
...
**Limits:** tiny suite (5 prompts × 2). Not Agent Bench. Quality champion remains published **94/100**, not re-run today.

### What we actually packaged today

1. Methodology fix: true control = clean upstream Original Qwen, not knobs-off on our binary  
2. Single-agent sequential A/B (above)  
3. Real-world Treebeard smokes: Broadway 6/6, long-ctx 3/3 + 14/14  
4. Docs/release evidence pack  

### What we did *not* prove today

- Smarter model (same weights)  
- Serious single-agent quality A/B on Agent Bench  
